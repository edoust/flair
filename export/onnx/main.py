...
def main():
    ...
    exportModel, forward, forwardIndices, backward, backwardIndices, characterLengths, lengths, forwardDictionary, backwardDictionary, outputTags = creator.createExportModel(modelName, sentences, with_whitespace, reverseForwardAndBackward)
    ...
    forward, forwardIndices, backward, backwardIndices, characterLengths, lengths))
    out = traced_model(forward, forwardIndices, backward, backwardIndices, characterLengths, lengths)

    print(out)

    # Export
    in_names = ["forward", "forwardIndices", "backward", "backwardIndices", "characterLengths", "lengths"]
    out_names = ["scores", "prediction"]

    torch.onnx.export(model=traced_model,
                      args=(forward, forwardIndices, backward, backwardIndices, characterLengths, lengths),
